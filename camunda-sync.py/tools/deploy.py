...
# -*- coding: utf-8 -*-
"""
🚀 BPMN Deploy Tool - Развертывание схем в Camunda Platform

НАЗНАЧЕНИЕ:
    Отправляет BPMN диаграммы в Camunda Platform через REST API для создания новых
    процессов или обновления существующих. Выполняет валидацию перед развертыванием
    и предоставляет детальную информацию о результатах деплоя.

ИСПОЛЬЗОВАНИЕ:
    python deploy.py <input_file.bpmn>

ПРИМЕРЫ:
    # Деплой локального файла
    python deploy.py camunda_my_process.bpmn
    
    # Деплой файла из другой папки
    python deploy.py ../converted/process.bpmn
    
    # Деплой с полным путем
    python deploy.py C:\\Users\\user\\camunda_process.bpmn

РЕЗУЛЬТАТ:
    - Создается новый деплой в Camunda Platform
    - Отображается информация о развернутых процессах (ID, версия, исполняемость)
    - Выводятся ссылки на Camunda Cockpit для мониторинга

ТРЕБОВАНИЯ:
    - Python 3.6+
    - Модуль camunda_client.py в родительской папке
    - Переменные окружения в .env:
      * CAMUNDA_BASE_URL - URL сервера Camunda (например, https://camunda.company.com)
      * CAMUNDA_AUTH_USERNAME - имя пользователя
      * CAMUNDA_AUTH_PASSWORD - пароль

ПОДГОТОВКА:
    1. Убедитесь, что BPMN файл сконвертирован для Camunda (используйте convert.py)
    2. Проверьте доступность Camunda сервера
    3. Настройте учетные данные в .env файле

ПРИМЕЧАНИЯ:
    - Рекомендуется использовать файлы с префиксом 'camunda_' (после конвертации)
    - Скрипт выполняет проверку соединения перед деплоем
    - При ошибках выводится детальная диагностика
"""
