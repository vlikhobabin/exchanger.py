...
РЕЗУЛЬТАТ:
    Создаются два файла в папке ./bpmn:
...
ОСОБЕННОСТИ:
    ...
    - Безопасное сохранение в UTF-8 кодировке в папку ./bpmn
    - Автоматическое создание папки ./bpmn если она не существует
...
БЕЗОПАСНОСТЬ:
    - Все файлы сохраняются в папку ./bpmn (не перезаписывают системные файлы)
...
def save_diagram_xml(diagram_id: str) -> None:
    ...
    try:
        ...
        # Путь к файлу в папке bpmn (на уровень выше от camunda-sync.py)
        root_path = Path(__file__).parent.parent.parent
        bpmn_path = root_path / "bpmn"
        
        # Создаем папку bpmn если она не существует
        bpmn_path.mkdir(exist_ok=True)
        
        file_path = bpmn_path / filename
        ...
        try:
            ...
            assignees_file_path = bpmn_path / assignees_filename
