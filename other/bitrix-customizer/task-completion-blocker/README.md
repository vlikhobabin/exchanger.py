...
### Шаг 2: Развертывание PHP компонентов

```bash
# 2.1. Скопируйте PHP файлы на сервер
...
# 2.2. Проверьте синтаксис PHP (ОБЯЗАТЕЛЬНО!)
...
### Шаг 3: Проверка работы PHP блокировщика

```bash
# 3.1. Создайте тестовую задачу в Bitrix24:
...
#### 4.2. Скачайте правильный footer.php
...
#### 4.4. Правильное редактирование footer.php
...
#### 4.5. Безопасная загрузка footer.php
...
2. **Проверяйте синтаксис PHP** перед загрузкой на сервер
3. **Тестируйте каждый компонент** отдельно
4. **Используйте правильный шаблон** (/local/templates/bitrix24/)
5. **Проверяйте доступность сайта** после каждого изменения
...
**Версия: 1.3 (Стабильная версия после инцидента)**  
**Дата: 2025-01-13**  
**Статус: Протестировано в бою, все критические ошибки исправлены** 