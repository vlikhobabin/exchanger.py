...
## Архитектурные улучшения v2.3

### Camunda Multi-Tenancy

Camunda Worker поддерживает изоляцию задач по tenant ID:

- **Production**: `CAMUNDA_TENANT_ID=imenaProd` — обрабатывает только production задачи
- **Development**: `CAMUNDA_TENANT_ID=imenaDev` — обрабатывает только dev задачи
...
- **Camunda Multi-Tenancy** — изоляция задач по tenant ID
...
- [x] **Camunda Multi-Tenancy** - изоляция задач по tenant ID
